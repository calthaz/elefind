...
*/
	...
	this.process = function(){
		...
		for (var i=0; i<initialLength; i++) {
			...
			switch(action.title){
				case('editProfile'):
					var dataToSend = { editProfile : JSON.parse(window.localStorage.getItem("elefindUser"))};
					//var status = false;
					...
					break;
				case('changeLang'):
					var dataToSend = {changeLang:action.data}; 
					$.ajax({
...
function hasConnection() {
	...
	*/
    if(navigator.onLine == false || navigator.connection!=null&&navigator.connection.type==Connection.NONE){
    	return false;
    }else{
    	return true;
    }
